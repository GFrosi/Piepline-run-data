...
'''A script to generate the full json and generate one json per IP samples with its correspondending inputs'''

def main():
    
    gse_list = open(args.list)
    ...
    path_to_git = args.gitpath #path to git (Corresponding_Input_Json_IHEC/srr_data_struct.py)

    for line in gse_list:

        gse = line.strip()
        out_name = gse + '.sh'
        output = open(os.path.join(root_path,gse,out_name), 'w')
...
#SBATCH --job-name=""" + "Json_generator_" + gse + "\nmodule load python/3.7\n"\
        ...
        #the GEO_metadata/GSEs generated by GEO metadata script - filtered by histones. When we run the pipeline
        #for other samples, we will need to change this file
        command_one = "python " + os.path.join(path_to_chipseq_generator,"gsm_seq.py") + " " + "-t "\
            + os.path.join(path_to_metadata,"GEO_79k_metadata_ToJson.csv ")\
        ...
        command_three =  "python " + path_to_git + " " + "-t " + os.path.join("/home/frosig/scratch/chip-seq-ihec-test/SRR-samples/samples/2022-GEO-548-cctrol-validateCA/Histones_allDBs_CA_pred_comparisonDBs_24-05-22-worddist.tsv ")\
            + "-j " + path_save_json 
...
if __name__ == "__main__":
    ...
    parser.add_argument('-g', '--gitpath', action="store",
                            ...
                            help='Absolute path to the directory (i.e git repo Corresponding_Input_Json_IHEC with the srr_struct.py',
                            required=True
